...
bool intersectIsPlane; // flag to distinguish intersection shape
...
// (f)
// intersection test with bouded plane
float plane_intersect(ray myRay, plane pl) {
  vec3 N = pl.N; 
  float d = dot(N, pl.position); 
  float NC = dot(N, myRay.origin); 
  float NV = dot(N, myRay.direction); 
  float t = (d - NC) / NV; 
  // intersect with plane
  vec3 pt = myRay.origin + (t * myRay.direction); 

  float minX = pl.position.x - (pl.width/2.0); 
  float maxX = pl.position.x + (pl.width/2.0); 
  float minY = pl.position.y - (pl.height/2.0); 
  float maxY = pl.position.y + (pl.height/2.0); 
  float minZ = pl.position.z - (pl.width/2.0); 

  if (pt.x < minX || pt.x > maxX || pt.y < minY || pt.y > maxY ||myRay.direction.z > 0.0) {
    t = largeT + 1.0; 
  } else {
    intersectPlane = pl; 
  }
...
float nearestT(ray myRay)
{
  ...
  curr_t = plane_intersect(myRay, scene_plane); 
  if (curr_t < nearest_t) {
    nearest_t = curr_t; 
    intersectIsPlane = true;
    intersectPlane = scene_plane; 
  }
...
//      a shadow ray.  If the point is in shadow, return black. Otherwise return the diffuse shading.
  ...
  vec3 diffuseColor = min(max(dot(N, L), 0.0), 1.0) * surfColor;
...
vec3 rayCast3(ray myRay)             // return the color for this reflected ray
{
  ...
  if (nearest_t < largeT) {
    ...
    if (intersectIsPlane) { 
      // hit a plane
      P = myRay.origin + myRay.direction * nearest_t; 
      N = intersectPlane.N; 
      localColor = localShade(P, intersectPlane.N, intersectPlane.mtrl.color); 
    } else {
      // hit a sphere
      P = myRay.origin + nearest_t * myRay.direction;
...
vec3 rayCast2(ray myRay, int bounceCount)
{
  ...
  if (nearest_t < largeT) {
    ...
    if (intersectIsPlane) { 
      ...
      N = intersectPlane.N; 
      localColor = localShade(P, intersectPlane.N, intersectPlane.mtrl.color); 
      kReflect = intersectPlane.mtrl.kReflect; 
      kLocal = intersectPlane.mtrl.kLocal; 
    } else {
      // hit a sphere
      P = myRay.origin + nearest_t * myRay.direction;
      N = normalize(P - nearestSphere.position);
...
vec3 rayCast(ray myRay, int bounceCount)             // return color
{
  ...
  if (nearest_t < largeT) {       // hit an object?
    ...
    if (intersectIsPlane) { // hit a plane
      P = myRay.origin + myRay.direction * nearest_t; 
      N = intersectPlane.N; 
      localColor = localShade(P, intersectPlane.N, intersectPlane.mtrl.color); 
      kReflect = intersectPlane.mtrl.kReflect; 
...
void initialize()
{
  ...
  plane mirror; 
  mirror.mtrl.color = vec3(0.0, 0.2, 0.0); 
  mirror.mtrl.kReflect = 0.8; 
  mirror.mtrl.kLocal = 0.2; 
  mirror.N = normalize(vec3(0.0, -0.1, 1.0)); // point to camera
  mirror.width = 15.0; 
  mirror.height = 6.0; 
  mirror.position = vec3(-1.0, 3.0, -25.0); 
  scene_plane = mirror; 
}
